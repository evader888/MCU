...
static void mater_push(u8 i)
{
    ...
    push485buf[0]=0xDD;//帧起始符固定为0xDD  
    push485buf[1]=class_sen[add].add >> 8;//从机地址高8位  
    push485buf[2]=class_sen[add].add;//从机地址低8位  
    push485buf[3]=0x02;//0x02代表RS-485网络数据  
    push485buf[4]=0x09;  
  
    //判断传感器类型，填入传感器类型代号  
    switch (class_sen[add].senty) {  
    case BodyInfrared_Sensor:       //人体红外  
        push485buf[5]=0x02;  
        break;  
    case Sound_Sensor:              //声音传感器  
        push485buf[5]=0x07;  
        break;  
    case Infrared_Sensor:           //红外传感器  
        push485buf[5]=0x08;  
        break;  
    //2. 模拟量  
    case Photosensitive_Sensor:     //光敏  
        push485buf[5]=0x06;  
        break;  
    case AirQuality_Sensor:         //空气质量  
        push485buf[5]=0x05;  
        break;  
    case Flame_Sensor:              //火焰传感器  
        push485buf[5]=0x03;  
        break;  
    case FlammableGas_Sensor:       //可燃气体  
        push485buf[5]=0x04;  
        break;  
    //3.温湿度  
    case TemHum_Sensor:             //温湿度  
        push485buf[5]=0x01;  
        break;  
    default:  
        push485buf[5]=0x10;  
    ...
    push485buf[6]=(u8)(class_sen[add].value>>8);  //传感器数据高8位  
    push485buf[7]=(u8)class_sen[add].value;       //传感器数据低8位  
    push485buf[8]=CHK(push485buf,8);
  
    RS4853_Send_Buffer(push485buf, 9);  
...
static void mater_push(u8 i)
{
    ...
    push485buf[6]=(u8)(class_sen[add].value>>8);
    push485buf[7]=(u8)class_sen[add].value;
    push485buf[8]=CHK(push485buf,8);
